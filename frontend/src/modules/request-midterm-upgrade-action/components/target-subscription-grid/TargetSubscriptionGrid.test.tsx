...
describe('getNewQuantityCell', () => {
  it('enables the input and forwards changes when recommended', () => {
    const onChange = jest.fn();
    const subscription = makeSubscription({ recommended: 'Yes', newQuantity: 7 });
    const { getByRole } = render(<>{getNewQuantityCell(subscription, onChange)}</>);
  ...
  it('disables the input when not recommended', () => {
    const { getByRole } = render(<>{getNewQuantityCell(makeSubscription({ recommended: 'No' }), jest.fn())}</>);
